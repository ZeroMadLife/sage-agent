...
def test_public_api_returns_no_store_citations_and_package_receipt() -> None:
    ...
    assert response.headers["x-public-package-revision"] == "2026-07-27.1"
    body = response.json()
...
def test_public_api_answers_the_minimum_identity_question_from_published_evidence() -> None:
    response = TestClient(_app()).post(
        "/api/public/v1/ask",
        json={"question": "你是谁？"},
    )
...
def test_public_api_streams_grounded_answer_stages_and_receipt() -> None:
    ...
    assert '"package_revision":"2026-07-27.1"' in body
...
def test_public_api_limits_by_socket_client_without_trusting_forwarded_header() -> None:
    ...
    assert limited.headers["x-public-package-revision"] == "2026-07-27.1"
...
def test_public_health_reports_package_and_configuration_state() -> None:
    ...
    assert response.json()["package_revision"] == "2026-07-27.1"
...
def test_public_api_follows_publish_and_revoke_without_process_restart(tmp_path: Path) -> None:
    ...
    assert before.json()["receipt"]["package_revision"] == "2026-07-27.1"

    payload = json.loads(Path("data/public/sage-public-v1.json").read_text(encoding="utf-8"))
    payload["revision"] = "2026-07-27.2"
    ...
    assert published.headers["x-public-package-revision"] == "2026-07-27.2"
    assert published.json()["receipt"]["package_revision"] == "2026-07-27.2"
    ...
    assert rolled_back.headers["x-public-package-revision"] == "2026-07-27.1"
    assert rolled_back.json()["receipt"]["package_revision"] == "2026-07-27.1"
