    ...
    <div class="journey-canvas">
      ...
      <details class="run-details">
        ...
        <section class="event-replay">
          <h2>事件回放</h2>
          <ul>
            <li v-for="stage in visitedEvents" :key="'eventId' in stage ? stage.eventId : stage.id">
              <time>{{ eventTime('timestamp' in stage ? stage.timestamp : stage.completedAt || stage.startedAt) }}</time>
        ...
        <section class="runtime-resources">
          ...
          <div v-if="projection.runtimeResources?.length" class="resource-grid">
            <button
              ...
              :disabled="!resource.operationRef"
              :aria-label="resource.operationRef ? `查看${resource.label}运行详情` : undefined"
              @click="resource.operationRef && emit('openOperation', resource.operationRef)"
            >
              ...
              <ChevronRight v-if="resource.operationRef" class="resource-open" :size="14" />
            </button>
          </div>
