...
import {
  ...
  type KnowledgeNodeResearchModel,
...
const selectedNodeResearchModel = computed<KnowledgeNodeResearchModel | null>(() => {
  if (!selectedNode.value) return null
  return buildKnowledgeNodeResearchModel({
...
<template>
  <div class="knowledge-workspace">
    <KnowledgeWorkspaceHeader
      :workspace-name="summary?.workspace_name"
    ...
    <div class="workspace-body" :class="{ 'is-loading': loading }">
      ...
      ></div>
      <aside class="knowledge-library compact" :class="{ open: libraryOpen }">
        <header>
      ...
      <div class="knowledge-canvas-shell" :class="{ 'has-selection': selectedNode || selectedPage }">
        ...
        <KnowledgeGraphToolbar
          ...
          :can-use-goal="goalNodeIds.length > 0"
          :can-use-local="Boolean(selectedNodeId)"
          :can-show-goal-path="Boolean(selectedGoalPath)"
          :show-goal-path="showGoalPath"
          @update:query="query = $event"
          @update:scope="selectGraphScope"
          @update:depth="graphDepth = $event"
          @update:color-mode="colorMode = $event"
          @toggle-kind="toggleKind"
          @toggle-goal-path="toggleGoalPath"
        />
        ...
        <template v-else-if="summary">
          <section v-if="mode === 'graph'" class="graph-stage" aria-labelledby="graph-title">
            <h1 id="graph-title" class="sr-only">{{ graphScopeLabel }}</h1>
            <KnowledgeGraphCanvas
        ...
        <button v-if="selectedNode || selectedPage" type="button" class="detail-backdrop" aria-label="关闭知识详情" @click="closeInspector"></button>
        <aside v-if="selectedNode || selectedPage" class="knowledge-detail-rail" aria-label="知识详情">
          ...
          />
          <KnowledgeNodeResearchPanel
            v-if="selectedNode && selectedNodeResearchModel"
            :model="selectedNodeResearchModel"
            :loading="nodeLoading"
            @choose="prepareNodeResearch"
          />
          <button
            v-else-if="selectedPage"
            class="continue-in-chat"
            type="button"
            @click="prepareNodeResearch('understand')"
          >
            在主对话中研究此页面
          </button>
        </aside>
...
.workspace-body,.workspace-body.is-loading { position:relative; display:grid; grid-template-columns:50px minmax(0,1fr); min-width:0; min-height:0; overflow:hidden; }
.knowledge-canvas-shell { position:relative; display:grid; grid-template-columns:minmax(0,1fr); min-width:0; min-height:0; height:100%; overflow:hidden; }.knowledge-canvas-shell.has-selection { grid-template-columns:minmax(0,1fr); }.detail-backdrop { position:absolute; z-index:7; inset:0; display:none; width:100%; height:100%; padding:0; border:0; background:var(--sage-overlay); }.knowledge-detail-rail { position:absolute; z-index:8; inset:0 0 0 auto; display:grid; grid-template-rows:minmax(0,1fr) auto; width:min(360px,100%); min-width:0; min-height:0; overflow:hidden; border-left:1px solid var(--sage-border); background:var(--sage-surface); box-shadow:var(--sage-shadow-drawer); }.knowledge-detail-rail :deep(.knowledge-inspector.compact) { position:relative; z-index:auto; inset:auto; width:100%; height:100%; box-shadow:none; }.continue-in-chat { align-self:start; display:flex; align-items:center; justify-content:center; gap:7px; width:calc(100% - 20px); height:46px; min-height:46px; margin:10px; border:0; border-radius:var(--sage-radius); color:white; background:var(--sage-brand-strong); font-size:var(--sage-font-sm); font-weight:650; }
...
.knowledge-library.compact { position:absolute; z-index:55; inset:0 auto 0 50px; width:min(286px,calc(100vw - 72px)); display:none; box-shadow:var(--sage-shadow-drawer); }.knowledge-library.compact.open { display:flex; }.library-nav.stage-tabs { display:grid; align-self:auto; }.library-nav.stage-tabs button { display:grid; min-height:36px; padding:0 8px; border-bottom:0; text-align:left; }
.knowledge-stage { min-width:0; min-height:0; height:100%; overflow:hidden; background:var(--sage-surface); }.stage-tabs { display:grid; gap:2px; min-width:0; }.stage-tabs button { min-height:36px; border:0; color:var(--sage-text-muted); background:transparent; font-size:var(--sage-font-sm); white-space:nowrap; }.stage-tabs button.active { color:var(--sage-brand-strong); background:var(--sage-brand-bg); font-weight:650; }
.graph-stage { position:relative; display:grid; grid-template-rows:minmax(0,1fr); height:calc(100% - 48px); min-height:0; }.graph-stage>.sr-only { position:absolute; width:1px; height:1px; overflow:hidden; clip:rect(0,0,0,0); }.list-stage h1 { margin:0; font-size:18px; letter-spacing:0; }
...
@media (max-width:1359px) {
  .knowledge-library.compact { position:absolute; z-index:55; inset:0 auto 0 0; width:min(280px,calc(100vw - 40px)); display:none; box-shadow:var(--sage-shadow-drawer); }.knowledge-library.compact.open { display:flex; }
}
@media (max-width:1100px) {
  .knowledge-detail-rail { width:min(360px,100%); }
}
@media (max-width:899px) {
  .knowledge-workspace { grid-template-rows:58px minmax(0,1fr) 38px; height:calc(100dvh - 64px - env(safe-area-inset-bottom)); }.workspace-body,.workspace-body.is-loading { display:block; }.knowledge-library.compact { left:0; }.knowledge-canvas-shell,.knowledge-stage { height:100%; }.detail-backdrop { display:block; }.workspace-footer span:nth-child(3),.workspace-footer span:nth-child(4) { display:none; }.list-stage { padding:18px 16px 36px; }.attention-grid { grid-template-columns:1fr; }
}
@media (max-width:620px) {
  .detail-backdrop { background:var(--sage-overlay); }.knowledge-detail-rail { inset:auto 0 0; width:100%; height:min(72dvh,620px); border-top:1px solid var(--sage-border); border-left:0; border-radius:var(--sage-radius-lg) var(--sage-radius-lg) 0 0; }.list-stage>header p { display:none; }.wiki-list article { grid-template-columns:minmax(0,1fr) 34px; }.wiki-list article>div { display:none; }.index-line,.migration-line { grid-template-columns:auto minmax(0,1fr); }.index-line code,.migration-line button { grid-column:2; justify-self:start; }.import-options { grid-template-columns:1fr; }.import-dialog>footer { display:block; }.import-dialog>footer p { margin-top:5px; text-align:left; }.workspace-footer { gap:10px; }.workspace-footer span:nth-child(2) { display:none; }
}
