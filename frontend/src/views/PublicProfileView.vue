...
const faqResponses: Array<{ keywords: string[]; answer: string }> = [
  {
    keywords: ['sage', '做什么', '项目'],
    answer: 'Sage 是一个 Personal AI Learning Companion：用户先设定目标，再让主对话结合个人知识和外部证据，安排练习并记录可验证进步。',
  },
  {
    keywords: ['harness', '恢复', '运行'],
    answer: 'Harness 2.0 把 planning、tool、approval、reply 和 terminal 统一投影到持久 Timeline；刷新或断线后，仍能恢复同一条运行上下文。',
  },
  {
    keywords: ['知识', '图谱', 'rag', 'wiki'],
    answer: 'Knowledge 页面主要呈现个人知识库的结构。真正的学习动作发生在主对话，选中的 graph node、page 和 revision 会作为提问上下文。',
  },
]
...
function answerQuestion() {
  ...
  if (!value) return
  const match = faqResponses.find((item) => item.keywords.some((keyword) => value.toLowerCase().includes(keyword)))
  agentMessages.value.push({ role: 'visitor', text: value })
  agentMessages.value.push({
    role: 'sage',
    text: match?.answer ?? '这版公开资料只覆盖 Sage、Harness 2.0 和 Knowledge 三个主题。你可以换一个更具体的问题。',
  })
  question.value = ''
}
...
<template>
  <div class="public-profile">
    <header class="public-header">
      <a class="public-brand" href="#top" @click="selectSection('home')">
      ...
      <nav aria-label="公开站点导航">
        <a :class="{ active: activeSection === 'work' }" href="#work" @click="selectSection('work')">项目</a>
        <a :class="{ active: activeSection === 'writing' }" href="#writing" @click="selectSection('writing')">笔记</a>
        <a :class="{ active: activeSection === 'path' }" href="#path" @click="selectSection('path')">轨迹</a>
        <a :class="{ active: activeSection === 'about' }" href="#about" @click="selectSection('about')">关于</a>
      </nav>
    ...
    <main>
      ...
      <template v-else>
        <section id="top" class="public-hero">
          <div class="hero-copy">
            ...
            <div class="hero-actions">
              <a class="primary-action" href="#work" @click="selectSection('work')">看 Sage <ArrowRight :size="15" /></a>
        ...
        <section id="writing" class="public-section writing-section">
          ...
          <div class="work-list">
            <a v-for="item in workItems" :key="item.index" class="work-row" :href="item.href" target="_blank" rel="noreferrer">
              ...
              <ArrowUpRight :size="17" class="row-arrow" />
            </a>
          </div>
    ...
    <aside v-if="drawerOpen" class="public-agent" aria-label="公开 Sage 助手">
      ...
      <section class="agent-body" aria-live="polite">
        <div v-for="(message, index) in agentMessages" :key="index" class="agent-message" :class="message.role"><span>{{ message.role === 'sage' ? 'Sage' : '你' }}</span><p>{{ message.text }}</p></div>
      </section>
      ...
      <form class="agent-form" @submit.prevent="answerQuestion"><textarea v-model="question" rows="3" placeholder="询问公开的项目与方法…"></textarea><button type="submit" aria-label="发送问题" title="发送问题"><Send :size="16" /></button></form>
...
.public-brand { display: inline-flex; align-items: center; justify-self: start; gap: 10px; color: #1a201c; text-decoration: none; }
.public-brand > span:last-child { display: grid; gap: 1px; }.public-brand strong { font-size: 14px; letter-spacing: .01em; }.public-brand small { color: #76847a; font-size: 10px; letter-spacing: .04em; }.brand-mark,.agent-mark { display: grid; place-items: center; width: 30px; height: 30px; color: #fff; background: #2f704e; }.brand-mark { border-radius: 50%; }
.public-header nav { display: flex; gap: 27px; }.public-header nav a { min-height: 35px; padding-top: 7px; border-bottom: 1px solid transparent; color: #637067; font-size: 12px; text-decoration: none; }.public-header nav a:hover,.public-header nav a.active { color: #1a201c; border-bottom-color: #2f704e; }
.ask-sage,.primary-action { display: inline-flex; align-items: center; justify-content: center; gap: 7px; min-height: 36px; padding: 0 12px; border: 1px solid #2f704e; color: #fff; background: #2f704e; font-size: 12px; text-decoration: none; }.ask-sage { justify-self: end; border-color: #cbd8cd; color: #28543c; background: #eef5ef; }.ask-sage:hover,.primary-action:hover { background: #255d41; }
.public-profile main { width: min(1240px, calc(100% - 64px)); margin: 0 auto; }.public-hero { display: grid; grid-template-columns: minmax(0, 1.02fr) minmax(440px, .98fr); gap: clamp(48px, 8vw, 132px); align-items: center; min-height: min(700px, calc(100svh - 78px)); padding: 68px 0 72px; }.hero-copy { max-width: 600px; }.hero-location,.eyebrow,.system-heading span,.proof-label { color: #7c8c81; font-size: 10px; letter-spacing: .12em; text-transform: uppercase; }.hero-location { margin: 0 0 20px; }.hero-copy h1 { margin: 0; font-size: clamp(48px, 7vw, 82px); letter-spacing: -.055em; line-height: .98; }.hero-lede { max-width: 520px; margin: 27px 0 0; font-size: clamp(22px, 2.3vw, 31px); line-height: 1.3; letter-spacing: -.025em; }.hero-support { max-width: 520px; margin: 19px 0 0; color: #6b796f; font-size: 14px; line-height: 1.8; }.hero-actions { display: flex; align-items: center; gap: 23px; margin-top: 28px; }.text-action { display: inline-flex; align-items: center; gap: 6px; padding: 0; border: 0; color: #2f704e; background: transparent; font-size: 12px; }.text-action:hover { color: #1a201c; }.hero-signals { display: flex; flex-wrap: wrap; gap: 18px; margin-top: 42px; color: #7b887f; font-size: 11px; }.hero-signals span { display: inline-flex; align-items: center; gap: 5px; }.hero-signals svg { color: #2f704e; }
...
.public-section { padding: 95px 0 104px; border-bottom: 1px solid #dfe6e0; }.section-heading { display: flex; align-items: end; justify-content: space-between; gap: 30px; margin-bottom: 44px; }.section-heading h2 { max-width: 590px; margin: 10px 0 0; font-size: clamp(27px, 3vw, 41px); letter-spacing: -.04em; line-height: 1.15; }.section-heading p { max-width: 260px; margin: 0; color: #728077; font-size: 12px; line-height: 1.7; }
...
.writing-section { padding-bottom: 94px; }.work-list { border-top: 1px solid #dfe6e0; }.work-row { display: grid; grid-template-columns: 54px minmax(0, 1fr) auto; align-items: start; gap: 18px; padding: 23px 0 25px; border-bottom: 1px solid #dfe6e0; color: #1a201c; text-decoration: none; }.work-row:hover .row-arrow { transform: translate(3px, -3px); color: #2f704e; }.work-index { padding-top: 3px; color: #9aa69d; font-family: var(--sage-font-mono); font-size: 11px; }.work-main { display: grid; gap: 7px; }.work-main > strong { font-size: 16px; font-weight: 650; }.work-main > small { max-width: 720px; color: #718077; font-size: 12px; line-height: 1.7; }.work-tags { display: flex; flex-wrap: wrap; gap: 7px; }.work-tags em { padding: 4px 7px; border: 1px solid #d3dfd5; color: #688070; background: #f2f7f3; font-size: 10px; font-style: normal; }.row-arrow { margin-top: 5px; color: #a5b0a7; transition: transform .2s ease, color .2s ease; }.notes-heading { display: flex; align-items: baseline; gap: 18px; margin-top: 64px; padding-bottom: 17px; border-bottom: 1px solid #dfe6e0; }.notes-heading strong { font-size: 13px; }.notes-list article { display: grid; grid-template-columns: 54px minmax(0, 1fr); gap: 18px; padding: 16px 0; border-bottom: 1px solid #e9eeea; }.notes-list time { color: #9aa69d; font-family: var(--sage-font-mono); font-size: 11px; }.notes-list p { margin: 0; color: #5e6d63; font-size: 13px; }
