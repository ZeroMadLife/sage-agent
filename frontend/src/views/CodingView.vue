...
import { Eye, EyeOff, FileText, History, ListTree, PanelRightOpen, ScrollText, Settings, SquarePen, X } from 'lucide-vue-next'
import {
  CodingComposer,
  CodingApprovalCard,
  CodingDiffDrawer,
            ...
            <section class="conversation-workspace">
              ...
              >
                <template #actions>
                  ...
                  <RouterLink class="new-chat-link" to="/assistant" aria-label="开始新对话" title="开始新对话"><SquarePen :size="15" /><span>新对话</span></RouterLink>
...
@media (max-width:767px) { .pane-left { width:100%; }.session-titlebar { padding:0 10px 0 56px; gap:8px; }.session-title-copy { flex:1; }.session-title-copy span { display:none; }.titlebar-actions { flex:none; gap:3px; }.titlebar-actions :deep(.git-badge),.process-toggle { display:none; } }
...
@container workbench-pane (max-width:720px) {
  .new-chat-link { width:30px; padding:0; }
  .new-chat-link span,.files-toggle { display:none; }
...
@media (max-width:1100px) {
  .new-chat-link { width:30px; padding:0; }
  .new-chat-link span,.titlebar-actions :deep(.git-badge) { display:none; }
  .facts-toggle { display:grid; }
}
@media (max-width:767px) {
  .sage-view { height:calc(100dvh - 64px - env(safe-area-inset-bottom)); }
  .conversation-workspace :deep(.composer.compact .composer-input textarea) { min-height:72px; }
