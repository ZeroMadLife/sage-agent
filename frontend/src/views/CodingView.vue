...
<style scoped>
.sage-view { --header-height:46px; --chat-content-max:1120px; position:fixed; inset:0; display:grid; grid-template-rows:auto auto minmax(0,1fr); min-width:320px; min-height:0; color:var(--sage-text); background:var(--sage-surface); overflow:hidden; }
.workbench-header { grid-row:1; display:flex; align-items:center; gap:9px; min-height:var(--header-height); padding:0 10px; border-bottom:1px solid #dfe3e8; background:#fff; }
...
.chat-shell { grid-row:3; display:grid; grid-template-columns:256px minmax(0,1fr); min-height:0; height:100%; overflow:hidden; }.pane-left,.pane-center { min-height:0; }.pane-left { overflow:hidden; }.pane-center { position:relative; display:grid; grid-template-rows:56px minmax(0,1fr) auto; min-width:0; min-height:0; background:#fff; }.session-titlebar { display:flex; align-items:center; justify-content:space-between; gap:16px; min-width:0; padding:0 clamp(16px,4vw,52px); border-bottom:1px solid #edf0f3; }.session-title-copy { display:flex; align-items:center; gap:9px; min-width:0; }.session-title-copy strong { min-width:0; overflow:hidden; color:#283342; font-size:14px; text-overflow:ellipsis; white-space:nowrap; }.session-title-copy span { display:inline-flex; align-items:center; flex:none; color:#748091; font-size:11px; }.session-title-copy span::before { width:6px; height:6px; margin-right:5px; border-radius:50%; background:#a7afb9; content:''; }.session-title-copy span.running { color:#137333; }.session-title-copy span.running::before { background:#16a34a; }.titlebar-actions { display:flex; align-items:center; justify-content:flex-end; gap:8px; min-width:0; }.files-toggle { display:inline-grid; place-items:center; width:30px; height:30px; padding:0; border:1px solid transparent; border-radius:6px; color:#52606f; background:#fff; }.files-toggle:hover { border-color:#d8dee6; color:#1d4ed8; background:#f4f7fb; }
