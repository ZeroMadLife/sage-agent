...
| **有界长书 Agentic RAG v1** | 主对话首轮 RAG 不足时最多并行 2 个只读 Research child；3 条可回答 recovery case 上，人工 oracle rewrite 把候选证据召回从 0.167 提到 0.889，EvidenceBundle 仍受 parent run 与 citation 约束；真实 LLMWiki generation runner 已就绪 | oracle 只测 recovery 上限，不是意图模型效果；无 gate 代理在无答案 case 上 false acceptance 为 1.00，真实生成、Faithfulness/RAGAS 数字仍待 Keychain provider 配置 |
