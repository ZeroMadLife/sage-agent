...
| **有界长书 Agentic RAG v1** | 主对话自动执行首轮 Knowledge RAG；不足时最多并行 2 个只读 Research child，EvidenceBundle 授权 Synthesize，第二轮无新 citation 或输出无引用时拒答；checkpoint/resume 不重复执行 | 已有确定性状态机/恢复/Eval 回归；尚无真实长书 retrieval、生成与端到端质量数字 |
