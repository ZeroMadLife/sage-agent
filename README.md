...
| 架构部分 | 当前职责 |
| --- | --- |
| **上下文管理** | 模型硬窗口与 32k 图工作集双层预算；长结果 offload 后按 session 分页回载，工具循环内可压缩 |
...
| **Context Budget v2** | 12 条确定性长工具任务；`32k/12k` 循环内压缩使累计模型输入估算下降 14.50%、checkpoint 消息内容下降 31.50%，扣除 4.94% 摘要成本后净 token 下降 9.57%；当前意图、工具调用对、决策标记与 Artifact 探针均 100% 保留/恢复 | 未评估真实摘要模型的语义质量、Provider 账单 token 与线上延迟 |
...
- Context Budget v2 的节省率来自确定性合成长工具任务和 provider-neutral 估算，不代表线上账单或回答质量。
