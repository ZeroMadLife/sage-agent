...
| **有界长书 Agentic RAG v1** | 主对话首轮 RAG 不足时最多并行 2 个只读 Research child；3 条可回答 recovery case 上，人工 oracle rewrite 把候选证据召回从 0.167 提到 0.889；clean source `742f131` 的 Top-10 真实生成收据中，12/14 完成 Judge，unsupported claim rate 0、citation correctness 1、hard negative correct abstention 1 | provider failure 为 2/14，完成集 Answer Claim Coverage/Correctness 为 0.500/0.500，context precision/recall 为 0.144/0.688，P50/P95 为 49.2/138.8 s；这些是 seed 诊断，不是生产 SLA；下一步是缺失 Claim 驱动的 rewrite、timeout/上下文优化 |
...
| **长书 4+2 KPI Scorecard v1（离线）** | 原子 Gold Claim Judge 已区分 Answer Correctness 与 Faithfulness；五种 clean FastEmbed/Top-10 策略同口径复跑后，contextual 的 Claim Coverage 0.733、P95 522 ms，成为 3s 预算内的离线候选；Top-10 generation 的 Answer Correctness 为 0.500 | provider failure 仍为 0.1429、端到端 P95 138.8 s，14 条 seed 未冻结；Scorecard 不修改线上 policy，先优化 provider、context budget 和 missing-claim rewrite |
