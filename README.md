...
| **有界长书 Agentic RAG v1** | 主对话首轮 RAG 不足时最多并行 2 个只读 Research child；3 条可回答 recovery case 上，人工 oracle rewrite 把候选证据召回从 0.167 提到 0.889；clean source `d442b37` 的 14-case 真实生成收据中，完成 case 的 unsupported claim rate 0、citation correctness 1、hard negative correct abstention 1 | 真实 run 的 provider failure 为 3/14，answerable 最终回答率为 4/7，context precision/recall 为 0.142/0.643，P50/P95 为 59.6/120.4 s；这些是 seed 诊断，不是生产 SLA；下一步是扩充 gold、claim-aware sufficiency 与 timeout/上下文优化 |
| **长书 Claim-aware Sufficiency v1（离线）** | 14 条 seed 拆为 15 个原子 claim，支持 `any/all` passage 绑定；Top-10 首轮 claim coverage / bundle completeness 为 0.733/0.700，真实 generation 完成子集为 0.619/0.571；当前完整 bundle 的离线 Readiness Precision/Recall 为 1/1 | 实际 rewrite 的 claim recovery gain 为 0，Faithfulness 1.0 也仅覆盖 4 个最终回答；gold 尚未独立 review，线上 Gate 保持未激活 |
| **长书 4+2 KPI Scorecard v1（离线）** | 原子 Gold Claim Judge 已区分 Answer Correctness 与 Faithfulness；五种 clean FastEmbed/Top-10 策略同口径复跑后，contextual 的 Claim Coverage 0.733、P95 522 ms，成为 3s 预算内的离线候选 | Answer Correctness 仍等待新版 Judge 真实复跑；14 条 seed 未冻结，Scorecard 不修改线上 policy |
