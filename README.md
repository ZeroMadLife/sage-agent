...
| **有界长书 Agentic RAG v1** | 主对话首轮 RAG 不足时最多并行 2 个只读 Research child；clean source `0123456` 的 missing-claim Top-10 真实生成收据中，14/14 完成 Judge，Answer Correctness 为 0.600，Claim Recovery Gain 首次达到 0.0333，hard negative correct abstention 1 | Recovery Gain 仍低于 0.05 且 resolution 为 0，P95 150.8 s、总 token 297,954；这些是 seed 诊断，不是生产 SLA，下一步是 recovery admission 与上下文/调用预算 |
...
| **长书 4+2 KPI + Provider Tradeoff v1（离线）** | 同一 Gold/contextual/Top-10 下，FastEmbed/豆包/百炼 Claim Coverage 为 0.733/0.833/0.700，隔离 P95 为 0.93/6.72/3.41 s；FastEmbed 保持默认，豆包是受延迟阻塞的质量候选；真实 generation 的 Answer Correctness 为 0.600、Provider Failure 为 0 | 豆包 2048 维超 3s 预算，百炼首轮覆盖没有超过本地基线；14 条 seed 未冻结，线上 policy 未修改，下一步是 PostgreSQL/降维和 recovery admission |
