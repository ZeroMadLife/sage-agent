...
H2.6B2 已完成发布门禁，待 PR 合入 `dev/sage-v7`。Evidence Bundle 只能读取当前父 run 中成功 Research child 的 durable receipt，并再次执行来源去重与 token 截断；Synthesize 仅拥有 `read_evidence_bundle`，没有网络、文件、Shell、写入或递归委派能力。同一 run 使用 opaque query/source fingerprint 阻止重复检索，新 run 仍允许刷新时效性证据。合入前仍不得描述为已发布。
...
第一批次没有加入 Web Search、Memory 或自动 Goal 续跑，先建立了后续扩展共享的能力底座。随后 H2.4B-C 已完成统一发现、安全提升、健康度与审计；H2.5A 已交付 Search 与当前回答引用，H2.5B0-B2 已依次交付运行预算与 Shell 可靠性、安全 Fetch/Artifact、异步 PDF 解析；H2.5C 已交付用户确认的 Knowledge Source Proposal；H2.6A 已交付单个受限 Research Subagent；H2.6B1 已交付最多三个 child 的真实并行、父子共享预算、恢复幂等账本和 evidence refs 确定性去重。H2.6B2 已完成父 run 绑定的 Evidence Bundle、run 级重复检索 breaker 和只读 Synthesize 发布门禁，合入后进入 H2.6C Practice Profile。
