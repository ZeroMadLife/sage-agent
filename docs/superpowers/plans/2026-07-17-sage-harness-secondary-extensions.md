...
H2.6B2 已合入 `dev/sage-v7`。Evidence Bundle 只能读取当前父 run 中成功 Research child 的 durable receipt，并再次执行来源去重与 token 截断；Synthesize 仅拥有 `read_evidence_bundle`，没有网络、文件、Shell、写入或递归委派能力。同一 run 使用 opaque query/source fingerprint 阻止重复检索，新 run 仍允许刷新时效性证据。
...
实现状态（2026-07-19）：已在唯一 Harness Runtime 中注册服务端静态
`practice` profile，复用父会话 Tool Policy、Approval、Sandbox、workspace 和
预算。该 profile 只拥有本地发现、文件编辑和 Shell；禁止 Web、MCP、Knowledge、
Memory 与递归委派。同一父 Run 只允许一个写入型 Practice child。只有未掩盖的
确定性测试命令真实退出码生成 `code_test` 候选，候选进入 child trace 和父
delegation receipt，但不会写入尚未实现的 Mastery Ledger，也不会改变掌握度。
