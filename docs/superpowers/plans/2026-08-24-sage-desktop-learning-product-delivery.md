...
- **源码门禁**：D2.7 代码提交 `8f036cb`（包含 `25cd105` 的停止竞态修复）；Rust full `92 passed`
  （49 crate + 43 integration/doc），fmt 与 Clippy `-D warnings` 通过；临时 Keychain round-trip/cleanup
  通过；Python desktop `61 passed`，Vue focused `39 passed`，Vue serial full `552 passed`，标准与 public
  Vue production build 通过。真实隔离 Playwright 学习链路 `3 passed`。`git diff --check` 通过。
- **正式 artifact**：不复用此前任何 receipt；从 clean code HEAD
  `8f036cbb530d3fe9cc8fc2a7c47947139312af5c` 及全新输出目录运行唯一 bundle 入口，一次成功。receipt 位于
  `/private/tmp/sage-desktop-8f036cb-r1/desktop-bundle-receipt.json`，SHA-256
  `71d6ed941f46724609dc32a11622bec02085cf9d8eb3848a36dddc19d139ad62`；`source_dirty=false`、
  target `aarch64-apple-darwin`。冻结 sidecar 12 项真实 product smoke 与 `.app` 6 项 lifecycle smoke
  全部 `passed`。
- **manifest 与计数**：sidecar receipt SHA-256 为
  `2c61ec89ea4e185c256d929b894a7080f069c3d35e9730871ff1c9c01bacde21`；artifact manifest 逐项校验通过，
  仍为 247 个 regular files、22 个 symlinks、0 missing。sidecar readiness、模型 turn、SQLite RAG、
  checkpoint reopen、TLS、secret hygiene 与副作用工具 fail-closed 均来自冻结产物真实请求。
- **签名与安全**：host、launcher、sidecar 均为 thin arm64 Mach-O；app deep strict 与三个嵌套 executable
  strict codesign 全部通过，签名为 ad-hoc。product smoke sentinel、长 bearer token、通用 `sk-*`、
  private-key marker、Provider key 文件与 `.env` 均零命中；精确进程检查均为零残留。
- **收口状态**：当前代码 commit 为 `8f036cbb530d3fe9cc8fc2a7c47947139312af5c`，上述 receipt 已固定。
  当前仅完成本地验收，未 push、未建 PR、未合入，也未删除分支或 worktree；Developer ID、公证、DMG
  仍是后续发布工作。
