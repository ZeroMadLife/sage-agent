...
4. Inspector：摘要、来源追溯、关联概念、最新学习与置信度。

桌面常驻 Inspector，平板和手机使用 Drawer；底部 Activity Bar 展示持久摄取任务。正文 15-16px，导航与元数据不低于 12px。颜色只承担状态和图社区，不用装饰渐变或卡片堆叠。

交互稿已覆盖：图谱节点选择、类型/社区着色、低风险撤销、高风险审核入口、任务展开、移动导航和移动 Inspector。它是实现基线，不代表后端图谱和 RAG 已交付。
