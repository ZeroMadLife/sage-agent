...
配置；Tailscale 只能作为网络边界，不能替代应用授权。公开个人门面作为独立静态构建交付，
不携带 Sage 私有路由、API 客户端、环境变量或运行时凭据；它可以先于完整应用公网化部署，
但不因此放宽私人 Canary 的认证与网络边界。
...
公开门面过渡阶段：
sagecompanion.cn -> 主机 Caddy 80/443 -> 127.0.0.1:8081 -> 独立静态 public 容器
```
...
- **Public**：只提供公开个人门面的静态文件，独立构建、独立 Caddy、独立内部网络，监听
  `127.0.0.1:8081`；不注入生产 env，不连接 API、PostgreSQL、Redis 或私人 Knowledge。
...
- 将公开个人门面拆为单独 Vite 入口与静态容器，不加载私人应用路由。
- ECS 先以 `127.0.0.1:8081` 验证健康；域名、ICP、主机 Caddy 与 80/443 由用户确认后切换。
- 公开 Agent 的服务端能力延后到独立 public-only API，当前不接私人 Harness。
