...
- SSH 使用现有公网 IP host key 作为固定别名校验；即使通过 Tailscale DNS 连接，也不会
  接受未知主机指纹。
...
```bash
ssh -i ~/.ssh/id_ed25519_sage_deploy sage-deploy@sage-agent-canary.tail74531c.ts.net \
...
控制器不会切换公网 IP，也不会重启服务器。先恢复 Tailscale/SSH，再运行：
