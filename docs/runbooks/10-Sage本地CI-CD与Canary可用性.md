...
- API 镜像的 Debian 主仓库和安全仓库默认使用阿里云镜像并固定 IPv4；可通过
  `SAGE_DEBIAN_MIRROR`、`SAGE_DEBIAN_SECURITY_MIRROR` 覆盖，避免 rootless Docker 经官方源
  下载系统包时长期无输出并拖垮 SSH 部署会话。
- ECS 拉取固定开发分支遇到瞬时 TLS/网络错误时最多重试 3 次，每次间隔 10 秒；仍失败才计入
  连续部署失败并保留当前服务。
- HTTPS 探针显式绕过本机 HTTP 代理，避免 VPN/代理变量把 Tailnet 内地址误判为不可用。
- 合并提交的必需 CI 尚未产生或仍在运行时只记录 `waiting-ci`，不累计部署失败；检查明确失败、
  SSH 或 deployctl 失败连续 3 次后才暂停并发送一次中文告警，人工修复后重新启用。
