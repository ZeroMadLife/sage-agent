...
链路；当前部署已显式进入 `PR_CANARY`，验证通过的小型前端修复可以提交中文 Draft PR，
但不会自动合并。
...
- 尚未完成：截图、报告 Issue、审查后自动返工、PR 状态同步和 auto-merge canary；
- `SHADOW_WRITE` 只会在仓库外保存 7 天证据，不产生 commit、push、PR 或合并。
- `PR_CANARY` 每天最多创建 1 个中文 Draft PR，同时最多保留 1 个 Harness 自动 PR；
  Claude 审查后仍保持 Draft，不自动合并。
