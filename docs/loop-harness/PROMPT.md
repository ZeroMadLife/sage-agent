...
Reviewer 只读完整 diff、复现和验证证据，只返回 `PASS`、`REQUEST_CHANGES` 或 `BLOCK`。
Reviewer 不编辑、不补救失败，也不能覆盖 Controller 的风险分类。`DRY_RUN` 和
`SHADOW_WRITE` 不启动 Reviewer；`PR_CANARY` 和 `AUTO_MERGE_TIER_A` 由 Controller 通过独立
synthetic relay session 触发 `sage-loop-review` Claude。Reviewer 只能读取本轮 artifact 目录中的
`shadow.patch` 和 `validation.json`，不能运行命令、联网或调用 GitHub。

Reviewer 的 `PASS` 只是必要门禁，不是合并授权。是否满足 Tier A、GitHub checks、精确
head/base 和人工阻止状态只能由 Controller 决定；任何高风险、范围不明或证据不足必须返回
`REQUEST_CHANGES` 或 `BLOCK`。
