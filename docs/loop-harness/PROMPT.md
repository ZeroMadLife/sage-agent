...
`SHADOW_WRITE` 不启动 Reviewer；仅 `PR_CANARY` 由 Controller 通过独立 synthetic relay
session 触发 `sage-loop-review` Claude。Reviewer 只能读取本轮 artifact 目录中的
