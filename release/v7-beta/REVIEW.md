...
当前结论是 **适合本地使用与受控私测，尚不满足无条件公网开放**。主要原因不是界面，
而是生产 Sandbox admission、云端 Knowledge tenant scope 和发布运维门禁仍未闭合。
...
公开主页只读取精选展示数据并回答有限静态问题。它不复用主账号的私有文件权限，后续
即使接入公开 Agent，也应使用独立 corpus、tool allowlist、预算和审计记录。
...
- **本地开发与受控私测**：可继续。
- **公开成长主页静态展示**：可继续，部署时必须使用 HTTPS 与独立公开数据边界。
