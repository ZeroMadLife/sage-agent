...
│  ├── middleware/ - 可测试顺序的 AgentMiddleware 链             │
...
| KnowledgeStore | Wiki/Source/Revision/Citation | 被 Agent 直接写入（必须走 proposal） | SQLite + Git 文件 |
...
**为什么默认切换后仍保留双轨**：旧 session 的 history、trace、diff 使用 legacy 协议，
静默迁移会改变历史解释。新会话走 v2，已有 legacy session 仍按原 profile 回放和回归。
...
Pico、Claude Code、DeerFlow 等项目可以帮助提出分层问题，但本章不维护功能排行榜。
外部项目会持续变化，且公开行为不一定等于内部实现。Sage 的三层结构必须由本仓库的
route、adapter、runtime、store 和测试证明；需要比较时，应基于明确版本的一手源码或
官方文档另做记录。
...
当前 Beta 以服务端配置的 workspace 和受控私测为主。独立云 workspace 与本地 companion
属于后续设计方向，只有代码、隔离测试和部署门禁完成后才能计入发布能力。
...
> [!warning] v7-beta 的三层边界已经成型，但不是所有边界都已完成生产验证
> - `deerflow_v2` 是新会话默认 profile，legacy 仅用于历史 session 与回归
> - 默认 registry 当前包含 8 个最小安全 middleware；其他能力由 adapter/runtime 承担
> - Container Sandbox 有契约和实现测试，但尚未完成目标生产环境的 admission 验证
> - Cloud workspace 有 ownership；Knowledge tenant scope 仍保持关闭
...
下一章：[Runtime 与 Engine](03-runtime-engine.md)
