...
## 和 Pico v3 / Claude Code / OpenClaw 的对标

| 维度 | Sage v7-beta | Pico v3 | Claude Code | OpenClaw |
| --- | --- | --- | --- | --- |
| Working Memory | from_session 反扫 | layered memory | - | - |
| Durable Memory | SQLite revisioned + CAS | file-based | CLAUDE.md | MEMORY.md |
| Dream/Reflection | proposal-only + policy gate | auto-dream | background extractor | DREAMS.md |
| 召回方式 | V6 index prefix / V6.7 deterministic recall | 预算截断 | async/budgeted | vector+keyword |
| 审批 | proposal approve/reject/rollback | 无（auto-write） | - | 人工审批 |
| Reflection Agent | tool-less + bounded evidence | - | - | read-only + proposal-only |

Sage 的 Dream proposal-only + MemoryPolicyEngine 设计来自 OpenClaw 的思路，比 Pico 的 auto-dream 更安全（Pico 会自动写入，有幻觉污染风险）。
...
- `tests/core/coding/test_memory_proposals.py` - proposal 状态机 + rollback
- `tests/core/coding/test_memory_recall.py` - 相关性召回（V6.7）
